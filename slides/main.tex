...
\author[Grupo 2]{{\normalsize\bfseries Integrantes}\\[2mm]
        Heitor Andrade Moura \and Francisco Cosme Monteiro Xavier\\
...
\begin{frame}{O modelo parece saber}
  ...
  \vspace{5mm}
  \centering
  O trabalho responde: como \alert{dar conhecimento} a um LLM,\\
  e como \alert{medir de verdade} o que mudou.
\end{frame}
...
\begin{frame}{Seis questões, um modelo, um domínio}
  \begin{center}
    \small
  ...
  \end{center}
  \vspace{3mm}
  \textbf{Setup comum}: Qwen3-4B, corpus DOM-PI $+$ docentesDC,
  Colab T4 \alert{grátis} (QLoRA 4-bit), benchmarks próprios com
  distratores reais.
  \vspace{2mm}

  {\footnotesize O notebook 01 mede o ``antes''; o 07 cruza todos os
   modelos com IC 95\%. Adapters no Hugging Face; notebooks executados
   com outputs preservados.}
\end{frame}
...
\begin{frame}{A métrica estava errada}
  ...
  \end{columns}
  \vspace{4mm}
  O cloze tem \alert{viés de comprimento} e subestimava o modelo.
  Confirmação independente: a perplexidade da resposta correta cai de
  \textbf{2796 para 7,05} após o SFT.
  \vspace{2mm}
...
\begin{frame}{Q5: RAG entrega o que falta, o fato}
  \begin{columns}[c]
    ...
    \column{0.44\textwidth}
      ...
      \end{itemize}
      \vspace{2mm}
      Com RAG as notas viram \alert{bimodais} (0 ou 5): o gargalo
      passa a ser o \textbf{recall do retriever}, não o gerador.
      \vspace{2mm}

      {\footnotesize O zero sem RAG também prova ausência de
       contaminação.}
  \end{columns}
\end{frame}
...
\begin{frame}{Q6: segurança sem treinar nada}
  ...
  {\footnotesize Os 2 escapes são paráfrases sem as palavras-chave:
   limite conhecido de filtros por regra.}
\end{frame}
...
\begin{frame}{Síntese: cada técnica resolve uma coisa}
  ...
  {\footnotesize\normalfont Limitações: benchmarks pequenos (IC largos),
   juiz da mesma família do gerador, RAG open-book, treinos enxutos na
   T4 grátis.}
\end{frame}
